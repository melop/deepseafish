...
library(GENESPACE)
runwd <- file.path("/data2/projects/dyao/compare/gene_loss_all_new/06_genespace_forGeneloss_GO/rundir")
list.files(runwd, recursive = T, full.names = F)

 gpar <- init_genespace(
   genomeIDs = c("BrotulaMultibarbata", "DictyosomaTongyeongensis", "PholisNebulosa", "Zebrafish", "Human"),
   speciesIDs = c("Brotula_multibarbata", "Dictyosoma_tongyeongensis", "Pholis_nebulosa", "Zebrafish", "Human"),
   versionIDs = c("1", "1", "1", "107", "107"),
   ploidy = rep(1,5),
   diamondMode = "default",
...
arrCol <- rainbow(24);
arrLabelGenomes <- c('BassozetusSp')

regs <- data.frame(
  genome = rep('BassozetusSp', 23),
  chr =1:23)
