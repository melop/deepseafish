...
$sPremadeFasta = "/public4/group_crf/home/cuirf/deepsea/cellpaper/exportaln/ret_improved_cleanDNA.fasta";
$sTreeFolder = "/public3/group_crf/home/cuirf/bahaha_assembly/hyphy/exportAln_genespace_nooutgroup/Relax_BTP";
$sOutDIR = "Relax_BTP";
$sGeneSymbolMap = "/public4/group_crf/home/cuirf/deepsea/cellpaper/orthofinder/orthologs.filtered.txt";
...
function fnProcessAln($sGeneName,$arrFullCodingSeq) {
  ...
  //CHANGE 2026-5-7, remove taxa with stop codon:
  $arrRemoveTaxa = array();
  //remove taxa consist of internal stop codons:
    if ($arrAlnRet["stopcodon"]) {
      $arrRemoveTaxa = $arrAlnRet['stoptaxa'];
      echo("Removing the following taxa due to internal stops: ".implode(", ", $arrAlnRet['stoptaxa'])."\n");
    }
    ...
    //remove taxa consist of only N's or gaps:
      ...
      if (array_key_exists($sRemoveTaxon, $arrFullCodingSeq)) {
			  unset($arrFullCodingSeq[$sRemoveTaxon]);
      }
		}
   
   $arrAlnRet = fnTranslateAlignment($arrFullCodingSeq , false); //remake alignment after exclusion
