...
$sMonoDesc = "taxagroup_desc.txt";
$sPremadeFasta = "ret_improved_cleanDNA.fasta";
$sOutDIR = "annotate_aa_changes.$sTaxon1.$sTaxon2";

$arrOutgroupGenera = array(
    "NonDeepSea"
);

$arrConvergenceGenera = array($sTaxon1, $sTaxon2);


$nMinTargetTaxaWithDerivedAAPerGenusPerc = 0.3; // float, 0 - 1, in each convergence genus, at least how many percentage of species need to have this convergent AA?
$nMinOutgroupSppCount = 5;
$nMaxAAStatesInOutgroup = 4; //maximum of 1 AA state in outgroup.
...
function fnProcessAln($sGeneName,$arrFullCodingSeq) {
	global $sOutDIR, $nMinOutgroupSppCount,$sMonoDesc, $nThisPart, $nTotalPart, $nGeneCount,$hOut, $arrOutgroupGenera , $arrConvergenceGenera, $nMinTargetTaxaWithDerivedAAPerGenusPerc, $nMaxAAStatesInOutgroup,$nTotalSitesExamined;
	...
	//	if (!$arrAlnRet["stopcodon"] ) { //there is no stop codon, do raxml
      ...
      //remove taxa that have internal stop codons:
      foreach($arrAlnRet['stoptaxa'] as $sTaxon) {
        unset($arrAAAln[$sTaxon]);
      }
      
      
      if ($arrAlnRet["stopcodon"]) {
        echo("$sGeneName\tTaxaRemovedDueToInternalStops\t".implode("\t", $arrAlnRet['stoptaxa'])."\n");
      }
      
      $oConstraintTree = fnParseMonoDesc($sMonoDesc, array_flip(array_keys($arrAAAln)) );
			//$sConstraintTree = fnTree2Newick($oConstraintTree , array());
			echo(count($arrAAAln) . " taxa included for $sGeneName \n" );
      ...
      if (count($arrOutgroupSpp) < $nMinOutgroupSppCount) {
        echo("$sGeneName\tOutgroup\tToo Few\n");
        return;
      }

      echo("$sGeneName\tOutgroup\t".implode("\t", $arrOutgroupSpp)."\n");
