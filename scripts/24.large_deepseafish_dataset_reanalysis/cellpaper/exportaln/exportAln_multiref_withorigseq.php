...
$sOrthologList = "../orthofinder/orthologs.filtered.txt"; //only look at single copy genes in Camellia, with outgroup duplicates masked
...
$sWriteMode = 'w';
$arrDoneOrths = array();
if (file_exists($sOutputPrefix."_cleanDNA.fasta" )) { //previous run found!
	$pPrevRun = popen("cat $sOutputPrefix"."_cleanDNA.fasta | grep '>' | cut -d':' -f2 | cut -d ';' -f1 | uniq", 'r');
	while(false!==($sLn = fgets($pPrevRun))) {
		$sLn = trim($sLn);
		if ($sLn == '') continue;
		$nOrth = intval($sLn);
		$arrDoneOrths[$nOrth] = true;
	}
	echo("Detected previous run, with ".count($arrDoneOrths)." genes done\nResume\n");
	$sWriteMode = 'a';
	//print_r($arrDoneOrths);
}
//die();
$hGenomeDef = fopen($sGenomeDef, "r");
//$hOutTable = fopen($sOutputPrefix."_counts.txt" , "w");
$hOutAA = fopen($sOutputPrefix."_AA.fasta" , $sWriteMode );
$hCleanOutAA = fopen($sOutputPrefix."_cleanAA.fasta" , $sWriteMode);
$hOutDNA = fopen($sOutputPrefix."_DNA.fasta" , $sWriteMode);
$hCleanOutDNA = fopen($sOutputPrefix."_cleanDNA.fasta" , $sWriteMode);
...
for($nOrth=0;$nOrth<count($arrOrthologs);$nOrth++ ) {
	...
	if (array_key_exists($nOrth, $arrDoneOrths)) {
		echo("$nOrth previously done, skip\n");
		continue;
	}
	...
	foreach( array_keys($arrOutgroupProt) as $sOutgroupSp ) {
		...
		if ($sIDString == 'NA') {
			echo("Warning: gene is NA in $sOutgroupSp, skipped\n");
			continue;
		}
		$sGeneEnsNCBIID = $sIDString; //new version, only contains mrna id
		$arrIDStrF = explode('|', $sIDString);
		if (count($arrIDStrF)==3) {
			list($s1, $s2, $sGeneEnsNCBIID) = explode('|', $sIDString); //old version of id format  species|spgeneid|mrna
      if ($s1 == "rna-gnl") {
        $sGeneEnsNCBIID = $sIDString;
      }
		}
	...
	if (count($arrFullCodingSeq2)<2) {
                        echo("Too few genes left, skipped\n");
                        continue;
